...
export async function generateMetadata({ params }: ProductPageParams): Promise<Metadata> {
  ...
  const [product, t] = await Promise.all([getProductById(id), getServerDictionary()]);
  return { title: product ? `${product.name} | ${t.common.brand}` : t.product.notFound };
}
    ...
    <section className="mx-auto max-w-5xl px-4 py-8">
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <div className="flex flex-col gap-3">
          <Chip label={t.categories[product.category] ?? product.category} size="small" className="!w-fit !bg-aqua-50 !text-aqua-700 dark:!bg-navy-500 dark:!text-aqua-100" />
          ...
          <p className="text-sm text-navy-300">
            {t.product.stock}: <span className="font-medium text-navy-900 dark:text-white">{product.stock} {t.product.stockUnit}</span>
          </p>
